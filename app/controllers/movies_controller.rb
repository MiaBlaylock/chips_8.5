class MoviesController < ApplicationController
  ...
  def search_tmdb
    if params[:search_terms].present?
      # test 1 case: supports RSpec test for string input
      @movies = Movie.find_in_tmdb(params[:search_terms])
      flash.now[:danger] = "No movies found with given parameters!" if @movies.blank?
      return render 'search_tmdb'
    end
  
    # test 2 case: supports rspec test for has input
